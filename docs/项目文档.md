...
| 类型 | 默认级别 | 触发 | 跳转目标 |
|---|---|---|---|
| `security_alert` | 紧急 | 账号安全、隐私风险类事件 | 通知中心 |
| `review_rejected` | 重要 | 驳回 | 条目详情页（含理由） |
| `review_changes` | 重要 | 要求修改 | 条目编辑页 |
| `appeal_result` | 重要 | 申诉结果 | 申诉详情 |
| `report_result` | 重要 | 举报处理完成（含 SLA 超时提醒管理员） | 举报详情 |
| `comment_hidden` | 重要 | 评论被隐藏 | 说明页 |
| `review_approved` | 普通 | 审核通过 | 条目详情页 |
| `comment_reply` | 普通 | 评论被回复 | 评论所在位置 |
| `spot_stale` | 普通 | 自己的条目被标记过期 | 条目编辑页 |

**重要程度分级**：`critical`（紧急）/ `important`（重要）/ `normal`（普通）/ `info`（提醒）。
`notify()` 调用方可通过 `level` 显式覆盖类型默认级别。

**三条触达通道**：

| 通道 | 投递时机 | 默认最低级别 | 静默时段行为 |
|---|---|---|---|
| 站内信 `inapp` | 通知产生时同步入库，始终必达 | 全部（凭证） | 不受影响 |
| 邮件 `email` | BullMQ `notify` 队列异步发送，失败指数退避重试 3 次 | 重要以上 | 非紧急暂缓，静默结束后**同一用户合并为一封摘要邮件**补发 |
| 浏览器推送 `push` | 同上；Web Push（VAPID）+ Service Worker | 普通以上 | 非紧急暂缓，静默结束后**逐条补发** |

- 每类外部通道在 `notifications` 表上有独立状态：`pending → sent / failed`，或 `deferred → sent`，或 `skipped`；`deferred_until` 记录补发时刻。
- Redis 不可用时 notify 队列入队失败，API 进程原地同步投递，外部通道不会停在 `pending`。
- 未配置 `VAPID_*` 时推送通道优雅降级为 `skipped`，站内信与邮件不受影响。
- 推送服务返回 404/410 的订阅标记 `expired`，由每日清理任务删除。
- `critical` 通知三通道齐发且**无视静默时段**（邮件主题加 `[紧急]` 前缀）。

**静默时段**：用户在设置中配置本地时区（IANA 名）下的 `[HH:MM, HH:MM)` 窗口，支持跨午夜与 DST；
worker 每 `NOTIFY_FLUSH_INTERVAL_MIN` 分钟（默认 5）巡检一次 `deferred_until <= now()` 的通知执行补发。

**用户设置项**（`user_settings`）：`notifyEmail / notifyPush / notifyInapp` 总开关，
`emailMinLevel / pushMinLevel` 各通道级别阈值，`quietHoursEnabled / quietHoursStart / quietHoursEnd / timeZone`。
订阅管理：`GET/POST/DELETE /me/push-subscription`，`GET /me/settings` 附带 VAPID 配置状态与公钥。

**处理**：站内信必达；邮件与浏览器推送按级别与用户设置分级发送。

**闭环出口**：通知可标记已读、全部已读（点击单条也会标记）、按级别筛选；点击直达对应位置；超过 90 天的通知自动归档清理。
...
-- ============ 通知 ============
CREATE TYPE notification_delivery_status AS ENUM ('pending','deferred','sent','skipped','failed');
CREATE TABLE notifications (
  ...
  read_at        TIMESTAMPTZ,
  inapp_status   notification_delivery_status NOT NULL DEFAULT 'skipped',
  email_status   notification_delivery_status NOT NULL DEFAULT 'skipped',
  push_status    notification_delivery_status NOT NULL DEFAULT 'skipped',
...
| 方法 | 路径 | 权限 | 说明 |
|---|---|---|---|
| GET | `/notifications` | user | 通知列表（可按 `level` 级别过滤） |
| POST | `/notifications/read` | user | 标记已读（可批量传 id，不传表示全部） |
| GET | `/me/settings` | user | 通知设置 + VAPID 配置状态与公钥 |
| PATCH | `/me/settings` | user | 修改通道开关、级别阈值、静默时段与时区 |
| GET | `/me/push-subscription` | user | 查询浏览器推送是否已订阅 |
| POST | `/me/push-subscription` | user | 注册浏览器推送订阅（endpoint + p256dh + auth） |
| DELETE | `/me/push-subscription` | user | 退订（可指定单设备 endpoint，不传表示全部） |
