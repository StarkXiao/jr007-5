...
**处理**：三条通道统一分级触达——站内信（通知中心）、邮件、浏览器推送（Web Push/VAPID）。

| 重要程度 | 通知类型 | 通道策略 |
|---|---|---|
| 重要 high | 审核驳回、申诉结果、评论被隐藏、SLA 超时告警 | 三通道；静默时段内也立即送达 |
| 普通 normal | 审核通过、要求修改、收到回复、举报结果 | 三通道；静默时段延后到静默结束补发 |
| 提醒 low | 条目过期 | 不单独打扰；静默结束或每天 09:00 聚合为一条摘要 |

每条通道（站内信/邮件/推送）都可在设置中单独开关；用户可设置按所在时区计算的静默时段（起止时刻，可跨午夜）。普通通知在静默期只入库不外发，静默结束后由 worker（每 10 分钟扫描 `deliver_at`）补发；低级别通知带 `digest_id` 聚合进摘要通知。浏览器推送订阅存于 `push_subscriptions`，推送服务返回 404/410 时自动清理失效订阅。
...
- **定时任务**（BullMQ 队列 + 调度）：
  ...
  - `notify_flush`：每 10 分钟补发静默期延后的通知，并把低级别通知聚合为摘要；
...
| 方法 | 路径 | 权限 | 说明 |
|---|---|---|---|
| GET | `/notifications` | user | 通知列表（含 level、已同步通道；隐藏的站内信不返回） |
| POST | `/notifications/read` | user | 标记已读（可批量传 id，不传表示全部） |
| GET | `/notifications/push/vapid-key` | user | 浏览器推送是否启用及 VAPID 公钥 |
| POST | `/notifications/push/subscriptions` | user | 登记/更新浏览器推送订阅 |
| GET | `/notifications/push/subscriptions` | user | 当前账号订阅数 |
| DELETE | `/notifications/push/subscriptions?endpoint=` | user | 撤销推送订阅（不传 endpoint 撤销全部） |
| GET | `/me/settings` | user | 读取设置（含三通道开关、静默时段、时区） |
| PATCH | `/me/settings` | user | 更新设置（静默起止为分钟数 0–1439，起止不能相同） |
...
- [ ] 已读/全部已读可用
- [ ] 邮件开关生效
- [ ] 重要/普通/提醒三级通知的通道策略正确（重要静默期即达、普通延后、提醒聚合摘要）
- [ ] 静默时段按用户时区生效，普通通知静默结束后补发
- [ ] 浏览器推送可订阅、可撤销，失效订阅自动清理
